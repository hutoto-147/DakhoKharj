...
# 1) Main navigation + clickable dashboard cards.
...
# 2) Installments: the InstallmentPlan is the single source of truth; do not create a second ReminderItem.
old_installment = '''                if (reminderKind == ReminderKind.INSTALLMENT) {
...
new_installment = '''                if (reminderKind == ReminderKind.INSTALLMENT) {
                    val amount = amountText.toLongAmountOrNull() ?: 0L
                    val count = installmentCount.toEnglishDigits().toIntOrNull() ?: 1
                    repo.saveInstallment(InstallmentPlan(title = installmentTitle, installmentAmount = amount, remainingCount = count.coerceAtLeast(1), nextDueAt = dueAt, accountName = accountName.ifBlank { "حساب اصلی" }, note = note, reminderDaysBefore = reminderOffset.coerceAtLeast(0)))
                    ReminderScheduler.scheduleAll(context)
                } else {'''
if old_installment not in text:
    fail('installment creation block was not found; repository may have changed')
text = text.replace(old_installment, new_installment, 1)
...
private fun SettingsScreen(repo: LedgerRepository, refreshToken: Int, onChanged: () -> Unit) {
    ...
    Column(
        ...
        SettingsAccordionSection("بانک، پیامک و حساب‌ها", "${accounts.size.toString().toPersianDigits()} حساب • ${bankImports.size.toString().toPersianDigits()} پیام در انتظار", openSection == "bank", { openSection = if (openSection == "bank") null else "bank" }) {
            Text("پیام بانکی ابتدا وارد صندوق بررسی می‌شود و بعد از تأیید به‌صورت درآمد یا هزینه ثبت می‌شود.", modifier = Modifier.fillMaxWidth(), textAlign = TextAlign.Start, fontSize = 12.sp)
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        SettingsAccordionSection("قفل و امنیت", if (repo.setting("pin_enabled", "0") == "1" || repo.setting("biometric_enabled", "0") == "1") "فعال" else "غیرفعال", openSection == "security", { openSection = if (openSection == "security") null else "security" }) {
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                Button(modifier = Modifier.weight(1f), onClick = { if (pinText.length < 4) status = "PIN باید حداقل ۴ رقم باشد." else { repo.setSetting("pin_hash", sha256(pinText)); repo.setSetting("pin_enabled", "1"); pinText = ""; status = "قفل PIN فعال شد."; onChanged() } }) { Text("فعال‌سازی PIN") }
...
object ReminderScheduler {
    ...
    fun cancelInstallment(context: Context, planId: Long) {
        cancel(context, INSTALLMENT_ID_BASE + planId)
    }
    ...
    fun scheduleAll(context: Context) {
        ...
        // نسخه‌های قدیمی برای هر قسط هم ReminderItem و هم alarm مستقل می‌ساختند.
        // آن رکوردهای legacy حذف می‌شوند تا هر قسط فقط یک alarm داشته باشد.
        reminders.filter { it.kind == ReminderKind.INSTALLMENT }.forEach { legacy ->
            cancel(context, legacy.id)
            repo.deleteReminder(legacy.id)
        }

        reminders
            .filter { it.kind != ReminderKind.INSTALLMENT && it.enabled && it.remindAt > System.currentTimeMillis() }
            .forEach { schedule(context, it) }

        repo.installments().forEach { plan -> scheduleInstallment(context, plan) }
    }
}
...
# 6) Signing: preserve stable *debug* signature for easy APK upgrades, never use the public test key for release.
if not GRADLE.exists():
    fail('app/build.gradle.kts not found')
gradle = GRADLE.read_text(encoding='utf-8')
gradle = gradle.replace('    signingConfigs {\n        create("stableDebug") {', '    // Public development key: DEBUG ONLY. Never use it to sign a production release.\n    signingConfigs {\n        create("stableDebug") {', 1)
release_sign_line = '            signingConfig = signingConfigs.getByName("stableDebug")\n'
# Remove only the release occurrence; debug still uses the stable dev key.
release_pos = gradle.find('        getByName("release") {')
if release_pos < 0:
    fail('release buildType not found')
pos = gradle.find(release_sign_line, release_pos)
if pos >= 0:
    gradle = gradle[:pos] + gradle[pos + len(release_sign_line):]
else:
    fail('release signingConfig line not found; repository may have changed')
GRADLE.write_text(gradle, encoding='utf-8')

print('KharjYar update applied successfully.')
