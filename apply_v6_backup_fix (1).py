...
# Switch debug signing to the persistent test key that already exists in the repository.
old_signing = """        // Public development key for io.github.hutoto147.dakhlokharj.debug only.
...
new_signing = """        // Public development key for io.github.hutoto147.dakhlokharj.debug only.
        // V6+ debug APKs can update each other; never use this key for Play release.
        create("stableDebug") {
...
if old_signing in g:
    g = replace_once(g, old_signing, new_signing, 'stable debug signing')
elif new_signing not in g:
    raise SystemExit('stable debug signing: source shape is unknown')
